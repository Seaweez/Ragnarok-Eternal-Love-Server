...
bool xTaskThreadPool::add(xNetProcessor *np)
{
  ...
  if (item)
  {
    ...
    ++item->m_dwConnectNum;
    XLOG_T("[Task thread pool],%u,distribute,epfd:%u, Current number of thread connections:%u, total connections:%u", np->getEpollFD(), item->m_pThread->getEpfd(), item->m_dwConnectNum, getCount());
    return true;
  }
...
void xTaskThreadPool::del(xNetProcessor *np)
{
  ...
  for (auto &it : m_list)
  {
    if (np->getEpollFD() == it->m_pThread->getEpfd())
    {
      ...
      {
        it->m_dwConnectNum--;
        XLOG_T("[Task thread pool],%u,Recycle,epfd:%u, Current number of thread connections:%u, total connections:%u", np->getEpollFD(), it->m_pThread->getEpfd(), it->m_dwConnectNum, getCount());
      }
      return;
    }
  }
