...
void SessionUser::checkDonate()
{
	TVecItemInfo vecItemInfo;
	xField *pField = thisServer->getDBConnPool().getField(REGION_DB, "donate");
		if(!pField)
			return;
	char where[64];
	bzero(where, sizeof(where));
	snprintf(where, sizeof(where), "accid=%llu and status=1", accid);
	
	xRecordSet set;
	QWORD ret = thisServer->getDBConnPool().exeSelect(pField, set, where, NULL);
	if (QWORD_MAX == ret)
	{
		XLOG << "[TEST1] charid" << id << XEND;
		return;
	}
	//XLOG << "[TEST2] charid" << id << XEND;
	//if (set.empty());
	//	return;
	//XLOG << "[TEST3] charid" << id << XEND;	
	const SDeposit* cfg = nullptr;
	for (DWORD i = 0; i < set.size(); ++i)
	{
		DWORD depositid = set[i].get<DWORD>("depositid");
		DWORD depositidcount = set[i].get<DWORD>("count");
		cfg = DepositConfig::getMe().getSDeposit(depositid);;
		if (cfg != nullptr)
		{
			ItemInfo info;
			info.set_id(cfg->itemId);
			//DWORD countDeposit = (cfg->count)+(cfg->count2);
			info.set_count(depositidcount);
			info.set_source(ESOURCE_OPERATE);
			vecItemInfo.push_back(info);
			
			setDepositVirgin(depositid, cfg->virginTag);
		}
	}
	
	//XLOG << "[TEST4] charid" << id << XEND;
    UserAddItemSocialCmd cmd;
    cmd.mutable_user()->set_charid(id);
    cmd.set_operatereward(EOperateType_Phone);
    for (auto &v : vecItemInfo)
    {
      cmd.add_items()->CopyFrom(v);
    }
	...
	if (1 != ret)
	{
		XLOG << "[checkDonate] ID Error " << id << XEND;
		return;
	}
	//XLOG << "[TEST] 领取" << id << "cmd" << cmd.ShortDebugString() << XEND;
  }
  //PROTOBUF(rev, send, len);
  //sendCmdToMe(send, len);
  //XLOG << "[TEST] 领取" << id << "ret" << rev.ShortDebugString() << XEND;
